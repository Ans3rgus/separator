...
class SeparatorModel:
    ...
    def initialize_level_pressure(self, level_liquid: float, pressure_gas: float):
        """
        Инициализация сепаратора по уровню жидкости и давлению газа
        """
        # Расчет объема жидкости
        self.state.volume_liquid = level_liquid * self.parameters.area
        self.state.mass_liquid = self.state.volume_liquid * self.fluid.liquid_density
        
        # Расчет объема газа
        self.state.volume_gas = max(0, self.parameters.volume - self.state.volume_liquid)
        
        # Расчет массы газа из уравнения состояния
        if self.state.volume_gas > 0:
            self.state.mass_gas = (pressure_gas * self.state.volume_gas * self.fluid.gas_molar_mass) / (self.fluid.R * self.fluid.temperature)
        else:
            self.state.mass_gas = 0
        
        # Установка давления газа
        self.state.pressure_gas = pressure_gas
        
        # Расчет уровня жидкости
        self.state.level_liquid = level_liquid
        
        # Расчет давления жидкости
        self.state.pressure_liquid = pressure_gas + self.fluid.liquid_density * level_liquid * 9.81
